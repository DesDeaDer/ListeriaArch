...
namespace UnityEngine.UI {
  ...
  [AddComponentMenu("UI/ImageL", 11)]
  public class ImageL : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter {
    public enum Type {
      Simple,
      Sliced,
      Tiled,
  ...
  public static class SetPropertyUtility {
    public delegate bool DSetColor(ref Color currentValue, Color newValue);
    public static bool SetColor(ref Color currentValue, Color newValue) => typeof(Image)
      .GetMethod<DSetColor>(nameof(SetPropertyUtility), SetColor)
      (ref currentValue, newValue);

    delegate bool DSetStruct<T>(ref T currentValue, T newValue) where T : struct;
    public static bool SetStruct<T>(ref T currentValue, T newValue) 
      where T : struct =>
      typeof(Image)
      .GetMethod<DSetStruct<T>>(nameof(SetPropertyUtility), SetStruct)
      (ref currentValue, newValue);

    delegate bool DSetClass<T>(ref T currentValue, T newValue) where T : class;
    public static bool SetClass<T>(ref T currentValue, T newValue)
      where T : class =>
      typeof(Image)
      .GetMethod<DSetClass<T>>(nameof(SetPropertyUtility), SetClass)
      (ref currentValue, newValue);
  }
}
